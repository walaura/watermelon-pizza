...
const BABUBO = await useFont(readFile("./fonts/babushka-bold.pxfont"));
...
const make = (heado, subbo) => {
  ...
  let ogImageLayer = transformLayer.setBackground(
    ...
    (index, data) => {
      ...
      return convertColor.fromRGB(...hslToRgb(BG_H_ % 1, BG_S_ % 1, BG_L_ % 1));

      return BG_COLOR + Math.random() * 20;
    },
  );
  console.log(ogImageLayer.x);
  ...
  return exportImage(
    composeLayer.overlay(target, ogImageLayer, {
      offset,
    }),
  );
};
...
const img = make(heado, subbo);

await writeFile("./img.bmp", img);
